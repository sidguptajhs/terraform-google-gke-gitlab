...
#   value       = google_container_cluster.services.location
...
#   description = "Host for the GKE cluster that services are deployed in."
# }

output "root_password" {
  value = var.gitlab_root_password != "" ? var.gitlab_root_password : random_string.autogenerated_gitlab_root_password.result
...
#   gcloud container clusters get-credentials services --zone ${var.region} --project ${var.project_id}
#   kubectl get secret pre-created-gitlab-initial-root-password -o go-template='{{ .data.password }}' | base64 -d && echo
#   EOF
